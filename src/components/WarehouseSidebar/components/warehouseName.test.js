...
describe('WarehouseName test', () => {
  it('Renders properly', () => {
    const { getByText } = render(<WarehouseName warehouse={warehouse} />);

    let element = getByText('Seattle');
    expect(element).toBeInTheDocument();
    expect(element).toHaveAttribute('href', '/sl/seattle-wa-1');
    element = getByText('(0.00 mi)');
    expect(element).toBeInTheDocument();
  });
});
